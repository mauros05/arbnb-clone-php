<main role="main">
  ...
  <div class="album py-5 bg-light">
    <div class="container">
      <div class="row">
      <?php for ($i = 0; $i < count($res["name"]); $i++){ ?>
        <div class="col-md-4">
          <div class="card mb-4 box-shadow card-apartment">
            ...
            <div class="card-body">
              <h3><?= $res["name"][$i] ?></h3>
              <p class="card-text"><?= substr($res["description"][$i],0, 60) ?></p>
              <div class="d-flex justify-content-between align-items-center">
                <div class="btn-group">
                  <a href="apartment.php?ac=s&idap=<?= $res["id_apartment"][$i] ?>" class="btn btn-sm btn-outline-secondary">View</a>
                </div>
                <span class="text-muted"><i class="fa-sharp fa-solid fa-dollar-sign"></i><small class="text-muted"><?= $res["price"][$i] ?></small></span>
              </div>
            </div>
          </div>
        </div>
